...
from lib.train.admin.stats import AverageMeter  # 导入自定义类
import torch
...
#distill:蒸馏
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        print("Warning: Network type mismatch. Skipping check.")
        ...
        if ignore_fields is None:
            ...
            # Never load the scheduler. It exists in older checkpoints.
        # 修改这里：添加 'optimizer' 到列表中，忽略优化器状态的加载
        ...
        for key in fields:
            ...
            if key == 'net':
                # 1. 获取 checkpoint 中的 state_dict
                state_dict = checkpoint_dict[key]

                # 2. 过滤掉记忆库相关的参数
                # 记忆库是运行时动态数据，形状会随训练变化，加载会导致形状不匹配错误
                # 我们直接删除这些键，让模型使用初始化的空记忆库
                keys_to_remove = [k for k in state_dict.keys() if 'memory_' in k]
                for k in keys_to_remove:
                    print(f"Ignoring shape-mismatched key: {k}")
                    del state_dict[k]

                # 3. 加载过滤后的权重
                net.load_state_dict(state_dict, strict=False)
            elif key == 'optimizer':
                self.optimizer.load_state_dict(checkpoint_dict[key],trict=False)
            else:
    ...
    def load_state_dict(self, checkpoint=None, distill=False):
        ...
        # assert net_type == checkpoint_dict['net_type'], 'Network is not of correct type.'
        print(f"Warning: Network type mismatch. Current: {net_type}, Checkpoint: {checkpoint_dict['net_type']}. Loading anyway.")
