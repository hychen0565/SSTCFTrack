...
import collections
import re  # 新增：替换原torch._six依赖的re模块

# ========== 核心修改1：移除torch._six，改用原生类型 ==========
string_classes = (str, bytes)  # PyTorch 2.x兼容的字符串类型定义
int_classes = int  # PyTorch 2.x直接使用原生int
...
def ltr_collate(batch):
    ...
    if isinstance(batch[0], torch.Tensor):
        ...
        if elem_type.__name__ == 'ndarray':
            # array of string classes and object
            # ========== 核心修改2：替换torch.utils.data.dataloader.re为原生re ==========
        ...
        if elem.shape == ():  # scalars
            py_type = float if elem.dtype.name.startswith('float') else int
            # ========== 核心修改3：替换torch.utils.data.dataloader.numpy_type_map为原生转换 ==========
            numpy_type_map = {
