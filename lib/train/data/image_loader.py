...
def default_image_loader(path):
    """The default image loader, reads the image from the given path.
    替换原jpeg4py逻辑为Pillow，失败则降级到opencv"""
    if default_image_loader.use_pillow is None:
        # 先尝试Pillow加载（替代原jpeg4py）
        im = jpeg4py_loader(path)  # 函数名保留，内部已替换为Pillow
        if im is None:
            default_image_loader.use_pillow = False
            print('Using opencv_loader instead.')
        else:
            default_image_loader.use_pillow = True
            return im
    if default_image_loader.use_pillow:
        return jpeg4py_loader(path)
    return opencv_loader(path)


# 替换原use_jpeg4py为use_pillow，逻辑完全对齐
default_image_loader.use_pillow = None


def jpeg4py_loader(path):
    """ 原jpeg4py_loader替换为Pillow实现，函数名保留以兼容调用 """
    try:
        with Image.open(path) as img:
            # 转换为RGB格式（与原jpeg4py.decode()输出格式一致）
            img = img.convert('RGB')
            return np.array(img)
    except Exception as e:
        print('ERROR: Could not read image "{}"'.format(path))
...
def hsijpg_loader(path):
    """ 替换jpeg4py为Pillow，保留原数据拼接逻辑 """
    try:
        # 用Pillow替代jpeg4py加载图片
        def load_img(p):
            with Image.open(p) as img:
                return np.array(img.convert('RGB'))

        im1 = load_img(path)
        im2 = load_img(path.replace('img1', 'img2'))
        im3 = load_img(path.replace('img1', 'img3'))

        # 保留原拼接逻辑，确保输出格式一致
        im = np.concatenate((
            im1[:, :, 2:3], im1[:, :, 1:2], im1[:, :, 0:1],
            im2[:, :, 2:3], im2[:, :, 1:2], im2[:, :, 0:1],
            im3[:, :, 1:2], im3[:, :, 0:1]
        ), axis=2)
        return im
    except Exception as e:
...
def hotjpg_loader(path):
    ...
    try:
        ...
        im2 = load_img(path.replace('img1', 'img2'))
        im3 = load_img(path.replace('img1', 'img3'))
        im4 = load_img(path.replace('img1', 'img4'))
        im5 = load_img(path.replace('img1', 'img5'))
        im6 = load_img(path.replace('img1', 'img6'))

        # 保留原拼接逻辑，确保输出格式一致
        im = np.concatenate((
            im1[:, :, 2:3], im1[:, :, 1:2], im1[:, :, 0:1],
            im2[:, :, 2:3], im2[:, :, 1:2], im2[:, :, 0:1],
            im3[:, :, 2:3], im3[:, :, 1:2], im3[:, :, 0:1],
            im4[:, :, 2:3], im4[:, :, 1:2], im4[:, :, 0:1],
            im5[:, :, 2:3], im5[:, :, 1:2], im5[:, :, 0:1],
            im6[:, :, 0:1]
        ), axis=2)
        return im
    except Exception as e:
